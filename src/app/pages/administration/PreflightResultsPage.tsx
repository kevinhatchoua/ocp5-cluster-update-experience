...
// Cluster update preflight results page
export default function PreflightResultsPage() {
  ...
  const handleExportPreflight = () => {
    const reportContent = `OpenShift Cluster Update Preflight Report
...
`;
    ...
    link.href = url;
    link.download = `cluster-update-preflight-${new Date().toISOString().split('T')[0]}.txt`;
  ...
  const ActionButtons = () => (
    <div className="flex gap-[12px] flex-wrap">
      ...
      >
        ...
        Export cluster update preflight
      </button>
      <button 
            ...
            <div className="space-y-[12px]">
              {clusterHealth.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-[12px] bg-white dark:bg-[rgba(255,255,255,0.03)] rounded-[8px]">
                  ...
                  <span className="px-[10px] py-[4px] bg-[#e8f5e9] dark:bg-[rgba(62,134,53,0.15)] text-[#3e8635] dark:text-[#81c784] rounded-[6px] text-[12px] font-semibold">
                    {item.status}
                  </span>
                </div>
              ))}
            ...
            <div className="space-y-[12px]">
              {workerNodes.map((node, index) => (
                <div key={index} className="flex items-center justify-between p-[12px] bg-white dark:bg-[rgba(255,255,255,0.03)] rounded-[8px] group">
                  ...
                  <div className="flex items-center gap-[12px]">
                    <span className="px-[10px] py-[4px] rounded-[6px] text-[12px] font-semibold bg-[#e7f1fa] dark:bg-[rgba(43,154,243,0.15)] text-[#0066cc] dark:text-[#4dabf7]">
                      {node.status}
                    </span>
                    <NodeKebabMenu
                      nodeName={node.name}
  ...
  return (
    <div className="h-full overflow-y-auto">
          ...
          <div className="flex items-center gap-[12px]">
            ...
            <div className="flex gap-[8px]">
              <button
                ...
                className={`px-[16px] py-[8px] rounded-[6px] font-semibold text-[14px] transition-colors flex items-center gap-[8px] ${
                  viewMode === "form"
      ...
      <LightSpeedPanel
        ...
        context="I can guide you through the update process step by step. The preflight checks show your cluster is healthy and ready for the update. What would you like to know?"
      />
    </div>
  );
}